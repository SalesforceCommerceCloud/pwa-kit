...
if (!semver.satisfies(foundNode, requiredNode)) {
  warnings.push(
    `- Node: ${foundNode} is installed, but we require ${requiredNode}`
  );
}

if (!semver.satisfies(foundNpm, requiredNpm)) {
  warnings.push(
    `- NPM: ${foundNpm} is installed, but we require ${requiredNpm}`
  );
}
...
if (warnings.length) {
  ...
  console.log(
    "To fix this warning, see: http://sfdc.co/pwa-kit-required-software"
  );
  rl.question(
    blue(

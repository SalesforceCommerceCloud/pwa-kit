...
program.option(
  "--fix",
  "try to fix errors by copying the right dependencies and configs packages"
);
...
const excludedPackages = ["devcenter"];
...
const check = () => {
  ...
  if (rootPkg.dependencies !== undefined) {
    errors.push(
      `The root package.json must not have any dependencies (only devDependencies)`
    );
  }
  ...
  listPackages().forEach((pkgDir) => {
    ...
    commonConfigs.forEach((name) => {
      ...
      if (!fs.readFileSync(src).equals(fs.readFileSync(dest))) {
        errors.push(
          `The config file at "${dest}" does not match the template in "${src}"`
        );
      }
    });
    ...
    Object.entries(peerDependenciesByPackage).forEach(
      ([localPackageName, localPeerDeps]) => {
        const dependsOnLocalPackage = !!(
          pkg.dependencies[localPackageName] ||
          pkg.devDependencies[localPackageName]
        );
        if (dependsOnLocalPackage) {
          Object.entries(localPeerDeps).forEach(
            ([requiredPackage, requiredRange]) => {
              const foundRange =
    ...
    const mistakenlySuppliedPeers = intersection(
      dependencies,
      peerDependencies
    );
    ...
    const peersNotInstalledForDev = difference(
      peerDependencies,
      devDependencies
    );

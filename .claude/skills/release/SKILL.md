...
- **commerce-sdk-react auto-stamps.** Its `version` lifecycle script (`packages/commerce-sdk-react/scripts/version.js`) prepends a dated header during the bump, and the machinery can fire it more than once — so this file is the one prone to stacked headers. Open it and **dedupe**: keep the one header matching the version you just set (verified above). If a stale header below it (older `-preview`, `-dev`) has bullets that belong in this release, **move those bullets up** under the kept header first — then delete the stale header line. Never delete a header without rehoming its bullets. Confirm no duplicate bullets remain across headers. (Sibling `scripts/version.js` files exist in pwa-kit-react-sdk/runtime/dev but are **not** wired to their `version` lifecycle, so they don't auto-stamp; if that ever changes, this step widens.)
- **All other changelogs need a manual header rewrite** — the bump only touched their `package.json`/lockfiles. Rewrite the top line:

...
export const checkoutProduct = async ({ page, userCredentials }) => {
  ...
  expect(createViolationFingerprints(accessibilityScanResults)).toMatchSnapshot(
    ["Checkout-a11y-snapshots", "review-order"]
  );
  ...
  const orderConfirmA11yScan = await new AxeBuilder({
    page,
  }).analyze();

  expect(createViolationFingerprints(orderConfirmA11yScan)).toMatchSnapshot([
    "Checkout-a11y-snapshots",
    "order-confirmation",
  ]);
};

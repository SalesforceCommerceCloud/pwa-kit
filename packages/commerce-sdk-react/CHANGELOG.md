## v5.3.0-dev
- [Feature] Support callable header values on `CommerceApiProvider` (`headers?: Record<string, string | (() => string | undefined)>`). Callables are resolved to strings at provider construction (e.g. `traceparent: () => getCurrentTraceparent()` for distributed tracing); entries resolving to `undefined`/empty are dropped. Resolved values feed the default SCAPI client config, the `Auth` client, and `ConfigContext`. Backward-compatible with existing string-only `headers`. [#3889](https://github.com/SalesforceCommerceCloud/pwa-kit/pull/3889)

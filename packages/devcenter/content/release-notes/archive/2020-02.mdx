...
-   Addressed an issue with the [Analytics Integrations](/apis-and-sdks/progressive-web-sdk/analytics-integrations/overview) library, which was tracking subsequent page loads inaccurately.
...
-   Addressed an issue with the [Salesforce Commerce Integrations Connector](/apis-and-sdks/commerce-integrations/connectors/sfcc), where product filtering was not returning results. The issue was the result of a [backwards-incompatible change](https://help.salesforce.com/articleView?id=b2c_20_1_W6869118_ocapi_product_search_comma_fix_as.htm&type=5) with the Salesforce OCAPI product search endpoint.

### Others

-   Resolved an issue in which Mobify Tag Preview was not functioning correctly on iOS 13.3.
-   Resolved a bug which was preventing Mobify partner developers from accessing log files for server-side rendered PWA projects.
-   Resolved a bug in the [Mobify Cloud API](/apis-and-sdks/mobify-cloud), in which targets that had previously been deleted were continuing to appear in the project’s list of targets.

...
This guide will introduce you to [Susy 3](https://oddbird.net/susy/docs/), the responsive grid library that Mobify includes in the [Progressive Web SDK](/apis-and-sdks/progressive-web-sdk). We'll discuss best practices for using the library and share sample code that you can use in your projects.
...
[Susy](https://oddbird.net/susy/) is a SASS library that helps you build readable and robust responsive grid systems. The library has been developed and maintained for over 8 years by [Miriam Suzanne](https://www.miriamsuzanne.com/), a prominent contributor in the web development community.
...
Susy is already installed for you as a Node package. In your project's `_variables.scss` file, you'll find a `$susy` Sass map that sets a couple defaults. [Read more about these options and others you can set](https://oddbird.net/susy/docs/config.html#variable--susy).
...
If you'd like, you can use Sass's [variable arguments](https://sass-lang.com/documentation/file.SASS_REFERENCE.html#variable_arguments) to group your breakpoint and config variables together. For example, we could group the `$large-breakpoint` together with the `$large-config`:
...
Use the [`@span`](https://oddbird.net/susy/docs/api.html#function--susy-span) function to set an element to span a number of columns. You can use it with `width`, `flex-basis`, `margin`, or any other CSS rule that accepts a number. If you pass a parameter larger than `1`, Susy will add in intermediate gutter spacing.
...
The Susy docs explain how to [add more parameters for context, nesting and other use cases](https://oddbird.net/susy/docs/api.html#function--susy-span).

In the above code sample, one **important caveat** to be aware of is that `width: span(4)` is actually relative to the width of the _parent container_. When you inspect the resulting CSS (e.g. `width: calc(96px + ((100% - 264px) / 8 * 4))`), you will see that it uses `100%` in its calculation. So whenever you use `span`, please be aware of where that gets used and the styling of its parent container.

_Note: Susy2 included a [`container` mixin](https://susy.readthedocs.io/toolkit/#container) that helped clearfix for your floats, but this was removed in Susy3. If you're using Susy with `float`, we've built our own `@include container` mixin into the SDK that provides similar functionality. It gets imported into your project in `app/stylesheet.scss` along with the other Susy mixins._

### Gutter

Use the [`@gutter`](https://oddbird.net/susy/docs/api.html#function--susy-gutter) function to add gutter math to a CSS rule. For example, adding `margin-right: gutter()` to a component will set the `margin-right` of that element to be the width of a gutter. We started setting up our layout in the example above, but we did not yet add gutter spacing between each instance of _cool-component_. We can use `gutter` to do that:
...
[Read more about the `gutter` function on the Susy docs.](https://oddbird.net/susy/docs/api.html#function--susy-gutter)

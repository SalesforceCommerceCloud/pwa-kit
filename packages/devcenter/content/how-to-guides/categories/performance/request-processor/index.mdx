---
title: Request Processor Tutorial
metaTitle: Request Processor Tutorial
metaDescription: Learn how to use Mobify's request processor control how requests are routed to your app.
category: Performance
---

<Callout type="info">

**Note:** This article discusses the request processor, which is only applicable to applications running on Mobify's Application Delivery Network.

To get the most out of this guide, you’ll want to have a good understanding of <a href="https://hpbn.co/" target="_blank">networking and caching</a>.

*Projects created before March 2019 do not include the request processor. For help adding it to your project, contact <a href="https://support.mobify.com/" target="_blank">Mobify Support →</a>*
...
This example uses `processRequest` to filter querystring parameters `gclid` and `utm_campaign`, which are commonly associated with Google marketing campaigns. It uses the [`QueryParameters`](../../../apis-and-sdks/progressive-web-sdk/utils/ssr-request-processing-utils#module-colon-progressive-web-sdk-slash-dist-slash-utils-slash-ssr-request-processing-utils-dot-QueryParameters) library function to simpify working with querystrings:
...
- Get the group from the request class using the `options` argument passed to `requestHook`

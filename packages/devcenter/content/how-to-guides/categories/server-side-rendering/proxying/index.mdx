---
title: Configuring Proxies for Mobify Platform Projects
metaTitle: Configuring Proxies for Mobify Platform Projects
metaDescription: Learn proxy behavior, and how to configure proxies for projects running on Mobify's Application Delivery Network.
featured: true
...
- Set the proxies using [Mobify API](../../../apis-and-sdks/mobify-cloud/rest-api)
...
> Proxies work both in local development and when deployed to a target. In local development, `proxyConfigs` is used to set up the `SSRServer`. This is managed by default in the scaffold. `proxyConfigs` is stored as part of a bundle.
...
For cloud targets, we recommend that you set the proxy settings in the Cloud via [Mobify API](../../../apis-and-sdks/mobify-cloud/rest-api). You'd want the Cloud (and not the bundle) to be the source of truth.
...
Instead, we can configure a target to override a bundle's `proxyConfigs` using the [Mobify API](../../../apis-and-sdks/mobify-cloud/rest-api).
...
Ideally, you'll only need to have 1 bundle for all cloud targets. You don't need to create a new bundle for each target just to modify the proxy settings. With [Mobify API](../../../apis-and-sdks/mobify-cloud/rest-api), set your proxy settings in the Cloud.

## Retrieving current proxy settings

Once the the proxy settings are set, later you can find out what they are by using the utility function [`getProxyConfigs()`](../../../apis-and-sdks/progressive-web-sdk/utils/universal-utils#module-colon-progressive-web-sdk-slash-dist-slash-utils-slash-universal-utils-dot-getProxyConfigs). For example, you might want to use different SFCC's OCAPI client IDs depending on which environment you're in.

---
title: Introduction to PWA Analytics
metaTitle: Introduction to PWA Analytics
metaDescription: "Get to know the differences between your PWA's analytics and traditional website analytics."
category: Analytics
minVersion: 2.0.0
maxVersion: 2.0.0
featured: false
...
In traditional websites, a large number of analytics scripts are loaded _synchronously_, which can negatively impact and performance of the site.
...
Whenever possible, analytics scripts should be loaded _asynchronously_ instead of synchronously. In general, in your PWA build, it’s best to only load an analytics script synchronously if it’s absolutely necessary.
...
**Note:** Do not use the history change trigger type. This trigger type listens for a browser’s history change in order to send a pageview event. This leaves no time for other page-related metrics, such as page title, to be set before sending a pageview event.

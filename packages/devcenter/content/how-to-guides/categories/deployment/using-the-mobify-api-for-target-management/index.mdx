---
title: Using the Mobify Cloud API for Target Management
metaTitle: Using the Mobify Cloud API for Target Management
...
The Mobify Cloud API allows you to interact with resources via a [RESTful](https://developer.mozilla.org/en-US/docs/Glossary/REST) interface:
...
**[Access our Mobify Cloud API Reference →](/apis-and-sdks/mobify-cloud)**
...
**Note:** To follow along in this tutorial, replace the Project ID with your own Project's ID. You can find it on your project's <strong>Settings</strong> page in [Mobify Cloud](https://cloud.mobify.com/).
...
[Access our list of Target API endpoints →](/apis-and-sdks/mobify-cloud)

## Another example: restricting target access to specific IPs

<Callout type="info">

**Note:** Use these steps only when your security policy requires you to restrict access to environments by IP. _In the general case, we recommend restricting access using [HTTP authorization](https://en.wikipedia.org/wiki/Basic_access_authentication) or a secret HTTP header checked in request processor or the app server._

</Callout>

Now let’s look at another example. Consider the `staging` target we created above, and imagine we wanted to restrict access to specific IPs. You can restrict access to `staging` using the Mobify Cloud API by updating its `ssr_whitelisted_ips`.
...
Note: IPs in `ssr_whitelisted_ips` use [CIDR format](https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing) and are comma separated.

...
![Server-side Rendered DNS Integration](server-side-rendered-dns-integration.png)
...
By default, this approach will serve the app to all visitors. (You can serve the app to a subset of traffic, for an A/B test for example, using the [request processor](/get-started/architecture/deployment-infrastructure/#request-processor).)
...
Your production target will already be setup as soon as you’ve created your project in [Mobify Cloud](/get-started/architecture/deployment-infrastructure/#mobify-cloud). That leaves the staging target. You can use the [Mobify Cloud API](/apis-and-sdks/mobify-cloud) to create your staging target and set its region. By default, the targets’ regions will be set to `us-east-1` (North Virginia, USA). Your targets should use the region closest to the data centers for your backend. We'd recommend allocating one hour to complete this step.
...
Now, we will configure targets so they’re ready to receive traffic. First we’ll do this for `staging`, as the staging target will help us to test the DNS cutover before we go live. To configure, update the properties `ssr_external_hostname` and `ssr_external_domain` using the [Mobify Cloud API](/apis-and-sdks/mobify-cloud-api).
...
- [Using the Mobify Cloud API for Target Management](/how-to-guides/categories/deployment/using-the-mobify-api-for-target-management)

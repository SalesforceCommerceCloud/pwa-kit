---
title: "Salesforce Commerce Concepts Lesson 4: Troubleshooting"
metaTitle: "Salesforce Commerce Concepts Lesson 4: Troubleshooting"
metaDescription: An advanced series to help you tackle the most challenging topics you may encounter while building your Mobify PWA with Salesforce B2C Commerce. Lesson four covers common issues that can arise while building a Mobify PWA with a Salesforce B2C Commerce backend.
category: Guides
---
...
The SDK's router works best when you can use a pattern in the URL to determine which template to render. Often this is not the case for various pages on Salesforce Commerce projects, especially category landing pages and product listing pages. On tag-loaded PWAs, we address this by using the [SelectorRouter](routing#the-selectorrouter-and-selectorroute-components). This router sends a request to the desktop site, and we can search the DOM of the desktop site for a particular selector. This is much slower than routing purely based off of the URL, so it should be used sparingly.

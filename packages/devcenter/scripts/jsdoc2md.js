...
const pwaSDK = path.join(__dirname, '..', '..', 'pwa-kit-react-sdk')

const packages = [
    {
        pkgName: 'pwa-kit-react-sdk',
        files: [
            path.join(pwaSDK, 'src', 'ssr', '**', '*.jsx'),
            path.join(pwaSDK, 'src', '**', '*.js')
        ]
    }
]

const main = () => {
    // Clean existing files in progressive-web-sdk

    packages.forEach(({pkgName}) => {
        const dirName = path.join(outputDir, pkgName.replace('@mobify/', ''))
        sh.find(dirName).filter(function(file) {
            if (file.match(/\.mdx$/)) {
                if (!file.includes('overview.mdx') && !file.includes('index.mdx')) {
                    sh.rm('-rf', file)
                }
            }
        })

...
- Storefront dev tasks (analyze, generate, refactor, modify, fix) → `retailapp_get_dev_guidelines` first, then:
  - Components → `retailapp_create_component`
  - Pages → `retailapp_create_page`
- Site or app performance or accessibility checks → `retailapp_run_site_test`.

If multiple apply (e.g., create then test), the order is: `retailapp_get_dev_guidelines` → creation tool(s) → `retailapp_run_site_test`.

Gating requirement:
- Before making ANY recommendations or remediation steps based on `retailapp_run_site_test` output, you MUST first call `retailapp_get_dev_guidelines` and incorporate its guidance. If `retailapp_get_dev_guidelines` has not been called yet in the current task flow, call it immediately before interpreting `retailapp_run_site_test` results. Do not suggest code or config changes from `retailapp_run_site_test` without this step.

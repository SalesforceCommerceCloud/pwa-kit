...
/*
 The ssr-shared-utils module is used in the PWA and in the SSR server. It
 ...
 * added by configureProxyConfigs in ssr-server-utils:
...
/**
 * If code is running in a UPWA, return true. If it's running
 * in any other context (PWA, AMP, etc), return false.
 * @function
 * @returns {boolean}
 */
export const isUniversal = () => window.Progressive && window.Progressive.isUniversal

/**
 * Get any SSR Performance Timer set on window.Progressive.
 *
 * If there is no timer, returns null.
 *
 * @private
 */
export const getSSRTimer = () =>
    (window.Progressive && window.Progressive.ssrPerformanceTimer) || null

...
out their suppression in the ssr-server-utils.test.js
`)
)
...
describe('ssr-server-utils tests', () => {
    ...
    describe('inSSRFiles', () => {
        updatePackageMobify(baseMobify)
        const buildDir = 'src/ssr/test_fixtures'
        setBuildDir(buildDir)
        const buildPath = path.resolve(process.cwd(), buildDir)

        const expectTrue = ['main.css', `${buildPath}/main.css`, 'main.js', 'vendor.js.map']
        expectTrue.forEach((filePath) =>
            test(`Expect ${filePath} to be matched`, () => expect(inSSRFiles(filePath)).toBe(true))
        )

        const expectFalse = ['mainline.js', 'no-op.js', 'static/no-op.js', 'no-op2.js']
        expectFalse.forEach((filePath) =>
            test(`Expect ${filePath} to not be matched`, () =>
                expect(inSSRFiles(filePath)).toBe(false))
        )
    })

    test('stripFileChunks', () => {
        updatePackageMobify(baseMobify)
        const buildDir = 'src/ssr/test_fixtures'
        setBuildDir(buildDir)
        const chunkPath = getWebpackChunkPath()
        const bundleUrl = getBundleBaseUrl()

        // These scripts are not listed in ssrOnly and ssrShared
        const scriptsToStrip = ['script-not-in-ssr.js', 'file://some/path/script.js']
        // These are scripts in ssrOnly and ssrShared
        const scriptsToKeep = ['ssr.js', 'ssr-loader.js', 'http://some.url/script.js']

        const headToStripFrom = document.createElement('head')

        // Add all the script tags to the head
        scriptsToKeep.concat(scriptsToStrip).forEach((s) => {
            const script = document.createElement('script')
            if (s.includes('//')) {
                script.src = s
            } else {
                script.src = `${chunkPath}${s}`
            }
            headToStripFrom.appendChild(script)
        })

        // stripFileChunks is changing the head in place, so we expect it to strip
        // out the script tags in scriptsToStrip
        const unlistedFiles = stripFileChunks(headToStripFrom)

        const scriptObjectsLeft = Array.from(headToStripFrom.getElementsByTagName('script'))
        const scriptsLeft = []

        scriptObjectsLeft.forEach((s) => {
            const str = s.getAttribute('src').slice(bundleUrl.length)
            scriptsLeft.push(str)
        })

        scriptsToStrip.forEach((s) => {
            // The unlistedFiles returned from stripFileChunks include only
            // files that start with buildOriginSlash
            if (!s.includes('//')) {
                expect(unlistedFiles, `Expect ${s} to be in unlistedFiles`).toContain(s)
            }
            expect(scriptsLeft, `Expect ${s} to be stripped out of the head`).not.toContain(s)
        })
    })

    test('scriptMap', () => {
        const jsPath = path.resolve(process.cwd(), 'src/ssr/test_fixtures/no-op.js')

        scriptMap[jsPath] = '123'
        expect(getScriptFile(jsPath)).toEqual('123')

        scriptMap[jsPath] = undefined
        const source = fs.readFileSync(jsPath, 'utf8')
        expect(getScriptFile(jsPath)).toEqual(source)

        const mapPath = `${jsPath}.map`
        const mapSource = fs.readFileSync(mapPath, 'utf8')
        expect(getSourceMapFile(jsPath)).toEqual({
            map: mapSource,
            url: jsPath
        })
    })
    ...
    describe('getBrowserSize', () => {
        const BROWSER_SIZE_NAMES = getBrowserSizeNames()
        ...
        const localDevices = [
            ...
            ...Object.keys(userAgents.phone).map((key) => ({
                ...
                remote: false,
                ua: userAgents.phone[key],
                expected: BROWSER_SIZE_NAMES.MOBILE
            })),
            ...
            ...Object.keys(userAgents.tablet).map((key) => ({
                name: `test for user agent key ${key}`,
                remote: false,
                ua: userAgents.tablet[key],
                expected: BROWSER_SIZE_NAMES.TABLET
            })),
            ...
            ...Object.keys(userAgents.desktop).map((key) => ({
                name: `test for user agent key ${key}`,
                remote: false,
                ua: userAgents.desktop[key],
                expected: BROWSER_SIZE_NAMES.DESKTOP
            }))
        ]
        ...
        const tests = [...localDevices, ...remoteDevices]

        tests.forEach((testConfig) => {
            test(`getBrowserSize (${testConfig.name})`, () => {
                const req = {
                    get: () => testConfig.ua,
                    header: (key) => testConfig.header[key]
                }
                setRemote(testConfig.remote)
                expect(getBrowserSize(req)).toEqual(testConfig.expected)
            })
        })
    })

    describe('detectDeviceTypes', () => {
        const localDevices = [
            // Test all mobile user agents
            ...Object.keys(userAgents.phone).map((key) => ({
                name: `test for user agent key ${key}`,
        ...
        tests.forEach((testConfig) => {
            test(`detectDeviceTypes (${testConfig.name})`, () => {
                ...
                setRemote(testConfig.remote)
                expect(detectDeviceTypes(req)).toEqual(testConfig.expected)
            })
        })

        test('should return device type based on query param device', () => {
            ...
            expect(detectDeviceTypes(req)).toEqual([TABLET])
        })
    })

    describe('setBrowserSizeNames', () => {
        test('should set the value returned by getBrowserSizeNames', () => {
            const originalValue = getBrowserSizeNames()
            const customValue = {
                MOBILE: 'CUSTOM_MOBILE_SIZE',
                TABLET: 'CUSTOME_TABLET_SIZE',
                DESKTOP: 'CUSTOME_DESKTOP_SIZE'
            }
            expect(
                originalValue,
                '`getBrowserSizeNames` should be the default value before `setBrowserSizeNames` is used'
            ).not.toMatchObject(customValue)

            setBrowserSizeNames(customValue)
            const newValue = getBrowserSizeNames()
            expect(
                newValue,
                '`getBrowserSizeNames` should be the new custom value after `setBrowserSizeNames` is used'
            ).toMatchObject(customValue)
        })
    })

    test('windowFetch rejects relative URLs when windowFetchOptions are unset', () => {
        const wfOptions = getWindowFetchOptions()
        wfOptions.appOrigin = undefined
        return windowFetch('/relative/path')
            .then(() => {
                expect(true, 'Did not expect windowFetch call to succeed').toBe(false)
            })
            .catch((err) => {
                expect(err.message.includes('Only absolute URLs are supported')).toBe(true)
            })
    })

    describe('optimizeCSS', () => {
        // Test cases for CSS optimization include all possible pairs of
        // path types (classNames) and quotes (quoteNames), further named value pairs
        const classNames = [
            'no-directory',
            'current-directory',
            'parent-directory',
        ...
        const quoteNames = ['single-quote', 'double-quote', 'no-quote']
        const classValues = ['', './', '../', 'file://', '//', '/']

        // Generate all CSS class pairs and URLs from value pairs
        const cssClasses = []
        const urlValues = []

        // If you need to change the test fixtures, you can use this
        // loop to generate them, e.g.:
        // - for HTML: `<div class="${className} ${quote}"></div>\n`
        // - for CSS:
        // `.${className}.${quote} {
        ...
        classNames.forEach((className, i) => {
            quoteNames.forEach((quote) => {
                cssClasses.push(`.${className}.${quote}`)

                let urlValue = `${classValues[i]}test-image.png`

                if (quote === 'single-quote') {
                    urlValue = `'${urlValue}'`
                } else if (quote === 'double-quote') {
                    urlValue = `"${urlValue}"`
                }
        ...
        const html = `
            <html>
                <head>
                    <style>
                        div {
                            background-image: url("./test-image2.png")
                        }
                    </style>
                </head>
                <body>
                    <div class="no-directory single-quote"></div>
                    <div class="no-directory double-quote"></div>
                    <div class="no-directory no-quote"></div>
                    <div class="current-directory single-quote"></div>
                    <div class="current-directory double-quote"></div>
                    <div class="current-directory no-quote"></div>
                    <div class="parent-directory single-quote"></div>
                    <div class="parent-directory double-quote"></div>
                    <div class="parent-directory no-quote"></div>
                    <div class="protocol single-quote"></div>
                    <div class="protocol double-quote"></div>
                    <div class="protocol no-quote"></div>
                    <div class="relative-protocol single-quote"></div>
                    <div class="relative-protocol double-quote"></div>
                    <div class="relative-protocol no-quote"></div>
                    <div class="absolute single-quote"></div>
                    <div class="absolute double-quote"></div>
                    <div class="absolute no-quote"></div>
                </body>
            </html>
        `

        // The CSS fixture has styles for *, html and body
        // as well as a background-image property for every value pair
        const cssPath = path.resolve(process.cwd(), 'src/ssr/test_fixtures/main2.css')

        // optimizeCSS is using the bundleBaseUrl for changing paths
        setBundleBaseUrl(false)

        test('optimizing the CSS changes the relative asset paths from main.css to absolute', () => {
            // 1 (text) + 3 (*, html and body) + 2 (inline CSS assertions) + 2 * 6 (classNames) * 3 (quoteNames)
            // (every time there is a new className add 6 assertions)
            expect.assertions(42)

            return optimizeCSS(html, cssPath).then((text) => {
                expect(text).toBeTruthy()

                // Expect the output CSS to contain styles for
                // *, html and body elements, as well all the value pairs
                const elements = ['*', 'html', 'body'].concat(cssClasses)
                elements.forEach((element) => {
                    expect(
                        text.includes(`${element} {`),
                        `Expected output CSS to contain styles for ${element}`
                    ).toBe(true)
                })

                // Expect the output CSS to not contain styles for div
                // because uncss is not optimizing inline CSS
                // and there are no styles for div in the styles fixture
                expect(
                    text.includes('div {'),
                    'Expected the output CSS to not contain styles for div'
                ).toBe(false)

                // Expect output CSS not to contain the inline CSS
                expect(
                    text.includes('test-image2.png'),
                    'Expected styles from inline CSS to not be in the output'
                ).toBe(false)

                // For each value pair, expect the path to not be changed if the
                // path includes a protocol or is absolute, otherwise expect the path to be changed
                cssClasses.forEach((className, i) => {
                    const isProtocolOrAbsolute =
                        className.includes('absolute') || className.includes('protocol')
                    expect(
                        text.includes(`url(${urlValues[i]})`),
                        `Expected the path in ${className} to${
                            isProtocolOrAbsolute ? ' not ' : ' '
                        }be changed`
                    ).toBe(isProtocolOrAbsolute)
                })
            })
        })
    })

    describe('shouldLoadResource checks if resource is allowed to load', () => {
        let resource

        const buildDir = '/Users/developer/myproject/build'
        const sandbox = sinon.sandbox.create()

        beforeEach(() => {
            reset()
            updatePackageMobify({
                ssrShared: ['vendors~checkout-payment.js*', 'windows-asset.js*']
            })
            resource = {
                url: {
                    href: 'mobify.com'
        ...
        test('returns false for any resources in allowedUrls = []', () => {
            const result = shouldLoadResource(resource, [])
            expect(result).toBe(false)
        })

        test('returns true for a resource matched on regex', () => {
            const result = shouldLoadResource(resource, [/mobify\.com/])
            expect(result).toBe(true)
        })

        test('returns true for a bundle script listed in ssrShared', () => {
            resource.url.href = `${getBuildOrigin()}/vendors~checkout-payment.js`
            resource.element.localName = 'script'
            const result = shouldLoadResource(resource)
            expect(result).toBe(true)
        })

        test('returns true for a bundle script map listed in ssrShared', () => {
            resource.url.href = `${getBuildOrigin()}/vendors~checkout-payment.js.map`
            const result = shouldLoadResource(resource)
            expect(result).toBe(true)
        })
    })

    describe('JSDOMCustomResourceLoader checks resources', () => {
        let url
        let options
        let callback
        let allowedUrls
        ...
        test('JSDOMCustomResourceLoader is NOT calling fetch for resources not in allowedUrls = []', () => {
            JSDOMCustomResourceLoader._fetch(url, options, allowedUrls, callback)
            expect(callback).not.toBeCalled()
        })

        test('jsdomCustomResourceLoader is loading a resource matched on regex', () => {
            allowedUrls = [/mobify\.com/]
            JSDOMCustomResourceLoader._fetch(url, options, allowedUrls, callback)
            expect(callback).toBeCalled()
        })

        test('jsdomCustomResourceLoader allows a bundle script listed in ssrShared', () => {
            url = `${getBuildOrigin()}/vendors~checkout-payment.js`
            options.element.localName = 'script'
            JSDOMCustomResourceLoader._fetch(url, options, allowedUrls, callback)
            expect(callback).toBeCalled()
            expect(consoleError.called).not.toBe(true)
        })

        test('jsdomCustomResourceLoader allows a bundle script map listed in ssrShared', () => {
            url = `${getBuildOrigin()}/vendors~checkout-payment.js.map`
            JSDOMCustomResourceLoader._fetch(url, options, allowedUrls, callback)
            expect(callback).toBeCalled()
            expect(consoleError.called).not.toBe(true)
        })

        test("jsdomCustomResourceLoader doesn't allow a bundle asset not listed in allowedUrls", () => {
            url = `${getBuildOrigin()}/vendors.png`
            JSDOMCustomResourceLoader._fetch(url, options, allowedUrls, callback)
            expect(callback).not.toBeCalled()
            expect(consoleError.called).not.toBe(true)
        })

        test('jsdomCustomResourceLoader allows a bundle script not listed in ssrShared, with a warning', () => {
            url = `${getBuildOrigin()}/vendors~checkout-basket.js`
            options.element.localName = 'script'
            JSDOMCustomResourceLoader._fetch(url, options, allowedUrls, callback)
            expect(callback).toBeCalled()
        ...
        test('jsdomCustomResourceLoader recognizes and loads resources with Windows paths', () => {
            const originalSep = path.sep
            const originalResolve = path.resolve

            try {
                // Stub path module with windows values/functions
                path.sep = '\\'
                path.resolve = path.win32.resolve

                // Set the build dir. If we don't do this, the private variables
                // used in `jsdomCustomResourceLoader` will be wrong
                const windowsBuildOrigin = 'C:\\path\\to\\project'
                setBuildDir(windowsBuildOrigin)

                // See: https://en.wikipedia.org/wiki/File_URI_scheme#Implementations
                url = `file:///C:/path/to/project/windows-asset.js`
                options.element.localName = 'script'
                JSDOMCustomResourceLoader._fetch(url, options, allowedUrls, callback)
            } finally {
                // Set path back to normal before verifying tests because for
                // some reason other tests will break unexpectedly...
        ...
        test('jsdomCustomResourceLoader uses timer', () => {
            url = `${getBuildOrigin()}/vendors~checkout-basket.js`
            options.element.localName = 'script'
            const timer = {
                start: sinon.stub(),
            ...
            return JSDOMCustomResourceLoader._fetch(
                ...
                expect(timer.end.called).toBe(true)
            })
        })

        test('jsdomCustomResourceLoader uses caching', () => {
            url = `${getBuildOrigin()}/vendors~checkout-basket.js`
            options.element.localName = 'script'

            return JSDOMCustomResourceLoader._fetch(url, options, allowedUrls, callback)
                .then(() => {
...
describe('ssr-shared-utils tests', () => {
    beforeEach(reset)
...
describe('setBuildDir', () => {
    test('should create paths in a predictable way, on Windows and Unix', () => {
        // These tests need to be run on Windows and must use path.resolve()
        // to verify correct handling across platforms.
        const temp = path.resolve(process.cwd(), 'temp')

        setBuildDir(temp)
        expect(getBuildOrigin()).toEqual(fileUrl(temp))
        expect(getWebpackChunkPath()).toEqual(`${fileUrl(temp)}/`)

        setBuildDir(temp + path.sep)
        expect(getBuildOrigin()).toEqual(fileUrl(temp))
        expect(getWebpackChunkPath()).toEqual(`${fileUrl(temp)}/`)

        const relativePath = ['path', 'without', 'starting', 'slash'].join(path.sep)
        const absolutePath = path.resolve(process.cwd(), relativePath)
        setBuildDir(relativePath)

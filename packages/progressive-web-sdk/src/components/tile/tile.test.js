...
test('renders image as primary', () => {
    const wrapper = mount(<Tile title="test" href="test.html" />)
    const primary = wrapper.find('.pw-tile__primary').first()
    expect(primary.length).toBe(1)
    expect(primary.prop('href')).toBe('test.html')
})

test('renders title as primary', () => {
    const wrapper = mount(<Tile title="test" href="test.html" />)

    const primary = wrapper.find('.pw-tile__primary').last()
    expect(primary.length).toBe(1)
    expect(primary.prop('href')).toBe('test.html')
})
...
test('renders primary as a link if href is present', () => {
    const wrapper = mount(<Tile href="test.html" />)

    const primary = wrapper
        .find('.pw-tile__primary')
        .first()
        .children()
    expect(primary.type()).toBe('a')
})

...
describe('Content Security Policy', () => {
    ...
    test('uses NODE_ENV to determine correct CSP to use', () => {
        process.env.NODE_ENV = 'development'
        const {directives} = getContentSecurityPolicy()
        expect(directives[FRAME_ANCESTORS]).toContain('localhost:*')
    })
    test('uses localhost for Runtime Admin in development', () => {
        const {directives} = getContentSecurityPolicy('development')
        expect(directives[FRAME_ANCESTORS]).toContain('localhost:*')
    })
    test('uses mobify-storefront.com for Runtime Admin in staging', () => {
        const {directives} = getContentSecurityPolicy('staging')
        expect(directives[FRAME_ANCESTORS]).toContain('https://*.mobify-storefront.com')
    })
    test('uses runtime.commercecloud.com for Runtime Admin in production', () => {
        const {directives} = getContentSecurityPolicy('production')
        expect(directives[FRAME_ANCESTORS]).toContain('https://runtime.commercecloud.com')
    })
    test("doesn't upgrade insecure requests for local development", () => {
        delete process.env.AWS_LAMBDA_FUNCTION_NAME // ensure `isRemote` is false
        const {directives} = getContentSecurityPolicy()
        expect(directives[UPGRADE_INSECURE_REQUESTS]).toBeNull()
    })
    test('upgrades insecure requests when deployed', () => {
        process.env.AWS_LAMBDA_FUNCTION_NAME = 'pretend-to-be-remote' // ensure `isRemote` is true
        const {directives} = getContentSecurityPolicy('staging')
        expect(directives[UPGRADE_INSECURE_REQUESTS]).toEqual([])
    })
    test('allows upgrade-insecure-requests directive to be replaced', () => {
    ...
    test("allows additional directives we don't use to be included", () => {
        const {directives} = getContentSecurityPolicy('development', {
            'la-trahison-des-test': ["Ceci n'est pas une CSP"]
        })
        expect(directives['la-trahison-des-test']).toEqual(["Ceci n'est pas une CSP"])
    })
    test('merges user-provided kebab-case directives with default directives', () => {
        const {directives} = getContentSecurityPolicy('development', {
            [FRAME_ANCESTORS]: ['example.com']
        })
        expect(directives[FRAME_ANCESTORS]).toContain('example.com')
    })
    ...
    test('does not merge duplicate user-provided directives', () => {
        expect(() => {
            getContentSecurityPolicy('development', {
                [FRAME_ANCESTORS]: [],
                frameAncestors: []
            })
        }).toThrow(/duplicate/i)

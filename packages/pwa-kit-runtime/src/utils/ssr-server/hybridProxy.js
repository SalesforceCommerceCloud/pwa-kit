...
export function hybridProxy(options) {
    ...
    // Gather MRT rules from options – any option whose key starts with 'hybridMRTRule'
    // is considered an MRT rule. This allows passing rules via server options instead of env vars.
    MRT_RULES = Object.entries(options)
        .filter(([key, value]) => key.startsWith('hybridMRTRule') && value)
        .map(([_key, value]) => value) // eslint-disable-line @typescript-eslint/no-unused-vars

    if (MRT_RULES.length === 0) {
        logger.warn(
            'WARNING: No MRT rules found. Please set options.hybridMRTRule* in your server options.'
        )
    }
    logger.info(`*********** MRT_RULES: ${MRT_RULES}`)
    PROXY_OPTIONS = {
        target: SFCC_ORIGIN,
        changeOrigin: true,
    ...
    return createProxyMiddleware(function (pathname, req) {
        ...
        // MRT eCDN rules are evaluated to determine what gets sent to MRT

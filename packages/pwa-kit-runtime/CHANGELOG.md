...
- HttpOnly session cookies: SLAS proxy now mirrors `customer_id`, `customer_type`, `enc_user_id`, `id_token`, `refresh_token_expires_in` (non-HttpOnly) and `idp_refresh_token` (HttpOnly) as siteId-suffixed cookies, and strips `idp_refresh_token` from the response body and upstream proxy requests [#3830](https://github.com/SalesforceCommerceCloud/pwa-kit/pull/3830)

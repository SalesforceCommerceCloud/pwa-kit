...
- **Bug Fix**: The SLAS Private Client Proxy now allow-lists the front-channel `oauth2/authorize` redirect (GET only, no credential injection), so social/IDP login is no longer rejected with a 403 (`Request to .../oauth2/authorize is not allowed through the SLAS Private Client Proxy`). The client secret is still only injected on the back-channel `oauth2/token` exchange.
## v3.20.0 (Aug 12, 2026)
...
- **Bug Fix**: HttpOnly session cookies with `commerceAPI.cookieDomain` set — the SLAS proxy now emits the host-scoped cookie deletion *before* the real `Domain=` cookie (previously after). On a cookieless SSR load, commerce-sdk-isomorphic reconstructs the guest-login token from the response's `Set-Cookie` headers using last-write-wins per cookie name; the previous order let the trailing empty host-scoped deletion clobber the real token, leaving the SSR shopper token empty and returning 401 on data-bearing routes (e.g. a PLP deep link). Browser behavior is unchanged because host-scoped and `Domain`-scoped cookies are keyed separately, so their relative order does not matter. The same reconstruction hazard affected the refresh token: both `cc-nx` and `cc-nx-g` map to `refresh_token` in the SSR parser, and the empty opposite-refresh-cookie deletion (emitted to switch between guest/registered) was appended *after* the real refresh write — so it is now emitted *before* it, keeping the reconstructed SSR refresh token intact. This refresh path was latent regardless of `cookieDomain`.
...
- Storefront Preview iframe (follow-on fix): the SLAS proxy response handler now also honors a client-sent `x-pwakit-preview-parent` header (re-validated against `STOREFRONT_PREVIEW_PARENT_ALLOW_LIST`) as an alternate trusted-preview signal when deciding session-cookie `SameSite`/`Partitioned` attributes. The `__Host-pwakit_preview_ctx` marker cookie set in 1/2 depends on the iframe document load reaching the origin, but that load is cacheable (`s-maxage`), so a CDN cache hit bypasses the marker writer and session cookies fall back to `SameSite=Lax` — blocked in the cross-site preview iframe. The header rides on the never-cached SLAS token request so it always reaches the origin; it is stripped before proxying to SLAS/SCAPI. Either signal (marker cookie or header) now suffices.
...
- HttpOnly session cookies: SCAPI proxy now normalizes the forwarded `Authorization` scheme to canonical `Bearer`. An incoming bearer matched case-insensitively (e.g. lowercase `bearer <jwt>` or a tab-separated scheme) is re-emitted as `Bearer <jwt>` — previously it was forwarded verbatim, and SCAPI rejects a non-canonical scheme (lowercase `bearer` → 401). The token is forwarded unchanged, so a fresh SSR bearer still wins over a stale cookie. Hardening/parity follow-on to the bearer pass-through narrowing above (no known live break, since the SDK always sends canonical `Bearer`); the same normalization must be mirrored in MRT's CloudFront Lambda@Edge. [#3859](https://github.com/SalesforceCommerceCloud/pwa-kit/pull/3859)
...
- HttpOnly session cookies: the `customer_id` and `customer_type` cookies now expire with the access token (aligned to the JWT `exp`, like `cc-at-expires`/`uido`/`id_token`) instead of the refresh-token TTL, and are (re)issued on every access-token response so they stay in sync with the current token. `usid` and `enc_user_id` remain refresh-TTL-aligned. [#3858](https://github.com/SalesforceCommerceCloud/pwa-kit/pull/3858)

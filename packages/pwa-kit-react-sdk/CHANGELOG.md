...
- [Feature] Distributed tracing for the SSR render path (production-gated on `getOTELConfig().enabled`). Creates W3C `ssr.render` server + `route-match`/`getProps`/`render-to-string` child spans parented on the incoming MRT `traceparent`, plus per-SCAPI-query `scapi:<name>` child spans under `getProps`; emits the HLD attribute set and exports span JSON to stdout via `MrtConsoleSpanExporter`. Exposes the active span's `traceparent` on `res.locals.traceparent` so the app can forward it on outbound SCAPI/SLAS calls. Uses a non-global propagator/provider so the existing `server_timing` path is untouched; off → no-op. SSR-initial-load only (client-side propagation tracked separately). [#3889](https://github.com/SalesforceCommerceCloud/pwa-kit/pull/3889)

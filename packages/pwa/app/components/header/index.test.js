...
const MockedComponent = ({history}) => {
    ...
    const onAccountClick = () => {
        ...
        if (customer.isRegistered) {
            history.push(getDefaultPathname('/account'))
        }
    }
    const onWishlistClick = () => {
        history.push(getDefaultPathname('/account/wishlist'))
    }
...
test('route to account page when an authenticated users click on account icon', async () => {
    ...
    await waitFor(() => {
        expect(history.push).toHaveBeenCalledWith(getDefaultPathname('/account'))
    })
...
test('route to wishlist page when an authenticated users click on wishlist icon', async () => {
    ...
    await waitFor(() => {
        expect(history.push).toHaveBeenCalledWith(getDefaultPathname('/account/wishlist'))
    })
})

            ...
            nodePath.node.declarations.forEach(declaration => {
                if (declaration.init && declaration.init.type === 'LogicalExpression') {
                    ...
                    if (!shouldKeep) {
                        ...
                        if (declaration.init.right && declaration.init.right.type === 'CallExpression' && 
                            ...
                            if (importArg.type === 'ArrowFunctionExpression' && 
                                ...
                                removeComponentCandidates.push(path.resolve(path.join(path.dirname(filePath), importPath)));
                            }
                        }
                        nodePath.remove();
                    } else {
                    ...
                    if (child.node.type === 'JSXExpressionContainer') {
                        ...
                        if (expr.type === 'LogicalExpression' || expr.type === 'ConditionalExpression') {
                            ...
                            try {
                                ...
                                console.error(`Error processing plugin logical expression: ${exprPath}`);
                            }
                        }
                    }
                });
...
function removeUnusedComponents(directory) {
    ...
    // Output results
    const filesToRemove = unusedFiles.filter(filePath => removeComponentCandidates.includes(filePath));
    if (filesToRemove.length > 0) {
        ...
        filesToRemove.forEach(file => {
            ...
            try {
                ...
                if (error.code === 'EPERM') {
                    console.log(`  ✗ Permission denied - cannot delete. You may need to run with sudo or check permissions.`);
                } else {

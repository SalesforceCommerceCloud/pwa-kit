...
- [Bugfix] Make `create-mobify-app-dev.js` resilient under Node 24 + Docker. Inherit Verdaccio's stdio (instead of piping through a `data` listener that the synchronous `lerna publish` can't drain), pin Verdaccio to `127.0.0.1:4873` in `local-npm-repo/config.yaml`, and probe readiness via a raw TCP connect (Node 24's `fetch` may resolve `localhost` to `::1` first). Also tolerate spurious non-zero exits from `npm run lerna -- publish from-package` when the captured output contains `Successfully published:`. Surfaced after the e2e-pr workflow moved into the Playwright Docker container.

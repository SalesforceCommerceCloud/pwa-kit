...
- [Bugfix] Inherit Verdaccio's stdio in `create-mobify-app-dev.js` and detect readiness by polling `/-/ping`, rather than piping stdout and reading from a `data` listener. The synchronous `lerna publish` that follows blocks the event loop for the entire prepare phase, so a piped Verdaccio fills its OS pipe buffer and Lerna's first publish request lands on a hung server (`ECONNREFUSED 127.0.0.1:4873`). Surfaced after the e2e-pr workflow moved into the Playwright Docker container.

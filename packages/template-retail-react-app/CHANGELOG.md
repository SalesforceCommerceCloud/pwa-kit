## v10.1.0-dev
- [Feature] Add a configurable `capabilitiesVersion` for the Commerce Client (Embedded Messaging) shopper-agent widget, forwarded to the bundle as `messagingConfig.capabilitiesVersion` and defaulting to `'65'`. Enables Embedded Messaging action progress indicators (capabilities version >= `63` / release 254); override per environment via the `capabilitiesVersion` field in `COMMERCE_AGENT_SETTINGS` / `config/default.js`.

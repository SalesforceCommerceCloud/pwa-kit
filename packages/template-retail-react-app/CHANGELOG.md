...
- [Bugfix] Deliver the Trusted Agent (Order on Behalf) result from the `/callback` page back to the opener. The server `/callback` handler previously returned an empty cached response for every request, so the React callback page never rendered. It now falls through to the renderer with `Cache-Control: no-store` when the request carries both `code` and `state` (the Trusted Agent redirect), while any other request still gets the cached empty response. When the popup lands on the same origin callback page, the page posts `code` and `state` to the opener via `postMessage` (scoped to its own origin) with a `BroadcastChannel` fallback, so login completes even when a `Cross-Origin-Opener-Policy: same-origin` header severs the popup. Requests with no `code` or `state` are ignored, so the standard SLAS redirect is unaffected.

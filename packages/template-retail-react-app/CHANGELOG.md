...
- [Bugfix] Send the shopper's SLAS access token to the Shopper Agent Token Bridge proxy (`/api/agent/identity/bridge`) in the `Authorization` header instead of the request body (non-HttpOnly mode). Bearer credentials belong in the `Authorization` header (RFC 6750), and request bodies are more likely to be captured by logs/APM than the auto-redacted `Authorization` header. HttpOnly mode is unchanged — the server still reads the token from the `cc-at_{siteId}` cookie. Any inbound `Authorization` header is ignored in HttpOnly mode.

...
- [Bugfix] Deliver the Trusted Agent (Order on Behalf) result from the `/callback` page back to the opener. When the popup lands on the same origin callback page with `code` and `state` in the URL, the page now posts them to the opener via `postMessage` (scoped to its own origin) with a `BroadcastChannel` fallback, so login completes even when a `Cross-Origin-Opener-Policy: same-origin` header severs the popup. The page ignores requests with no `code` or `state`, so the standard SLAS redirect is unaffected.

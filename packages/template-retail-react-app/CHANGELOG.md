...
- [Feature] Guest Order Lookup: 3-step OTP flow for unauthenticated order status and tracking (`/order-lookup`, `/order-lookup/verify`, `/order-lookup/order`). Feature-flagged via `app.guestOrderLookup.enabled`. Adds Express endpoints (`POST /api/order-lookup/verify`, `GET /api/order-lookup/order`), in-process IP throttle middleware, HttpOnly session cookie, server-side field suppression, and "Find Your Order" footer link. Requires ECOM 26.8 `guestOrderLookup` SCAPI endpoint. @W-23444948

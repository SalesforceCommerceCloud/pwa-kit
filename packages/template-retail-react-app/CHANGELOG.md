...
- [Feature] Add a full-PDP delivery-estimate calculator. Shoppers can enter a country code and postal code to retrieve the primary deliverable shipping option without modifying their basket; valid destinations are saved per site after a successful response. The calculator selects the lowest-priced eligible option, or the earliest delivery window when prices are unavailable, and does not expose provider non-delivery reasons. Increase the `main.js` bundle-size budget from 105 kB to 106 kB for the calculator. [#4008](https://github.com/SalesforceCommerceCloud/pwa-kit/pull/4008)

## v10.1.0-dev
- [Bugfix] Set `amount` when adding a credit-card payment instrument to the basket, and re-sync it whenever the basket's order total changes after a payment has been added (promo applied, shipping method changed, item added). Without these, the resulting `PaymentTransaction` is persisted with a null or stale amount; downstream OMS ingestion rejects null amounts with `Payment currency code [N/A] does not match order currency code [...]`, and stale amounts produce payment/order-total mismatches at place-order time.

## v10.1.0-dev
- [Bugfix] Set `amount` when adding a credit-card payment instrument to the basket. Without it, the resulting `PaymentTransaction` is persisted with a null amount and currency, which downstream OMS ingestion rejects with `Payment currency code [N/A] does not match order currency code [...]`.

## v10.1.0-dev
- [Feature] Add return-item-selection modal on order detail. Registered users can select which items to return, set quantity, and pick a reason from `getOmsMetaData`. Gated by the per-item `quantityAvailableToReturn` eligibility signal and a customer-ownership check on the order. The "Return Items" button on order detail now opens this modal (W-22821837).

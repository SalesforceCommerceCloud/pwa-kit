...
- [Feature] Add the `dw_attribution` first-touch marketing-attribution cookie (W-23493129). New SSR middleware writes a sanitized, allowlist-only campaign cookie on the catch-all render route so Salesforce CIP can attribute placed orders back to the campaign that drove a headless-storefront visit. The HttpOnly + Secure + `SameSite=Lax` cookie has a 30-minute write-once TTL and stores percent-encoded `key=value` pairs (`utm_source`/`utm_medium`/`utm_campaign`/`utm_term`/`utm_content`, `gclid`, `fbclid`, `msclkid`, and a sanitized external `ref`); no PII or free-text params are captured. It writes only when the deployment allows cookies (`MRT_ALLOW_COOKIES=true` remotely / `localAllowCookies` locally), honors Do Not Track (`dw_dnt=1`), is write-once (first touch wins), and forces `Cache-Control: no-store` on the write so the per-visit cookie is never shared-cached and replayed to other shoppers.

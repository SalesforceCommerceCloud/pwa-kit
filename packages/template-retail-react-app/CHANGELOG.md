...
- [Bugfix] Filter OMS-emitted shipping-cost surcharges from the order detail Items Ordered list, item count, return dialog, and order history item count. OMS emits a shipping-cost surcharge as a `productItems[]` entry with a shipping-method id (e.g. `UK_Ground`) in `productId` — it is not a returnable product but arrives shaped like one. Filtering is keyed on a Shopper Products `getProducts` batch lookup miss: any `productItems` entry whose `productId` doesn't resolve to a catalog product is dropped. Falls through to the raw list when the batch is empty (loading or transient outage) so a blank product response doesn't hide every line.

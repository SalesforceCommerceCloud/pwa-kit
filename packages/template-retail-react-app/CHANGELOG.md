## v10.1.0-dev
- [Feature] Add return review step and OMS submission to the return-items flow on the order detail page. After selecting items, registered users with OMS enabled see a read-only review view (item, quantity, reason) and submit the return via the ShopperOrders `returnOmsOrder` mutation, with inline submit-error/Retry handling and a "Return submitted" confirmation. Return feedback is kept separate from cancel feedback. [#3895](https://github.com/SalesforceCommerceCloud/pwa-kit/pull/3895)

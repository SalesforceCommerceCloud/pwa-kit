...
- [Feature] Add a full-PDP delivery-estimate calculator. Shoppers can enter a country code and postal code to retrieve the slowest eligible shipping option without modifying their basket; valid destinations are saved per site after a successful response. The calculator selects the latest delivery window and does not expose provider non-delivery reasons. Increase the `main.js` bundle-size budget from 105 kB to 107 kB for the calculator. [#4013](https://github.com/SalesforceCommerceCloud/pwa-kit/pull/4013)

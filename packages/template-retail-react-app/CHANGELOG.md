## v10.1.0-dev
- [Feature] Add the Cimulate Copilot widget as an optional shopper-agent provider. A new `commerceAgent.provider` config switch selects between `miaw` (default, Salesforce Embedded Messaging) and `cimulate`; when set to `cimulate`, the Cimulate messaging UMD bundle (allowlisted to `*.cimulate.ai`) is loaded and rendered in a configurable display mode (`panel`/`dialog`/`modal`). Existing MIAW storefronts are unaffected.

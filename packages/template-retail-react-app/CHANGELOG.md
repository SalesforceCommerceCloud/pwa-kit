## v10.1.0-dev
- [Feature] Integrate SCAPI Cancel Order API. Cancel modal now calls the real OMS cancel endpoint, fetches reason codes from OMS metadata API, and uses `quantityAvailableToCancel` for eligibility. Removes `app.oms.enabled` config flag — cancel visibility is driven by order OMS data presence. [#3873](https://github.com/SalesforceCommerceCloud/pwa-kit/pull/3873)

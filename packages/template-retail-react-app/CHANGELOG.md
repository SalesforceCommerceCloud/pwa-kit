...
- [Feature] Add `app_pwakit_base` B2C cartridge for transactional email delivery via ISML templates and `dw/net/Mail`. GLO access-code emails are delivered via the `sfcc.app.order.sendOrderAccessCode` B2C hook; passwordless login and OTP verification emails are delivered via the cartridge's `pwakit-notify` Custom REST API. Includes ISML templates for GLO access code, passwordless magic link, OTP verification, and password reset. **Deployment note:** after uploading the cartridge to WebDAV and adding `app_pwakit_base` to the site cartridge path, deactivate and reactivate the code version in Business Manager (Administration → Code Deployment) so the SFCC runtime registers the hook and the SCAPI Custom API endpoint. Also import `OrganizationPreferences.xml` via Administration → Site Development → Import & Export to register the `pwakitNotifyEnabled` and `pwakitStorefrontHosts` custom preferences. [#XXXX](https://github.com/SalesforceCommerceCloud/pwa-kit/pull/XXXX)
- [Bugfix] Fix GLO `/api/order-lookup/verify` and related server endpoints returning 401 when `enablePWAKitPrivateClient` is enabled. The `cc-nx-g_` cookie stores the SLAS refresh token, not the access token — the access token is never stored in any cookie. All GLO server endpoints now read the SLAS JWT from the `Authorization` header sent by the client (via `useAccessToken().getTokenWhenReady()`), which works correctly in both public and private client modes. [#XXXX](https://github.com/SalesforceCommerceCloud/pwa-kit/pull/XXXX)

...
- [Feature] Add guest order lookup Step 1 (request) and Step 2 (verify) pages with full form wiring: Step 1 collects order number + email and fires `requestOrderAccessCode`; orderNo is placed in the URL path param for bookmark/refresh resilience while email travels only in router state (never in any URL) to avoid PII exposure. Step 2 reads orderNo from `useParams()` (survives hard refresh), renders a 2-line skeleton in place of the email subtext when email is absent, and POSTs to `/api/order-lookup/verify` with the SLAS token on submission. Anti-enumeration: Step 1 always routes to Step 2 regardless of whether the order/email pair is valid. On successful verify, navigates to `/order-lookup/order/:orderNo`. [#3951](https://github.com/SalesforceCommerceCloud/pwa-kit/pull/3951)

...
- [Feature] Add `app_pwakit_base` B2C cartridge for transactional email delivery via ISML templates and `dw/net/Mail`; replace Marketing Cloud email delivery with SLAS callback mode that calls the cartridge's `pwakit-notify` SCAPI Custom API (`c_pwakit_notify` scope); add GLO magic-link auto-submit on the verify page when `?token=` is present in the URL. **Deployment note:** after uploading the cartridge and adding it to the site cartridge path, you must deactivate and reactivate the code version in Business Manager (Administration → Code Deployment) so that the SCAPI runtime rescans and registers the `pwakit-notify` endpoint — SCAPI only discovers Custom API endpoints at code version activation time, so uploading to an already-active version is not sufficient. [#XXXX](https://github.com/SalesforceCommerceCloud/pwa-kit/pull/XXXX)

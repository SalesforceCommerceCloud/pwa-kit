## v10.1.0-dev
- [Feature] Wire the distributed-tracing `traceparent` into the `CommerceApiProvider` `headers` prop so outbound SCAPI/SLAS calls made during SSR carry the current trace context. Resolved via a server-only `getCurrentTraceparent()` (gated by `WEBPACK_TARGET`/`typeof window` so no OpenTelemetry is bundled into the client); no-op when tracing is disabled. [#3889](https://github.com/SalesforceCommerceCloud/pwa-kit/pull/3889)

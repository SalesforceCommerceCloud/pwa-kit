...
- [Bugfix] Preserve a literal `+` in the email query param on the reset-password landing page. A raw `+` decodes to a space via `URLSearchParams`, which corrupted plus-addressed emails (e.g. `name+tag@example.com`) and caused SLAS to reject the reset with a 400 "Invalid hashed verifier code". The `+` is now re-encoded as `%2B` before parsing so it survives as a literal `+`.

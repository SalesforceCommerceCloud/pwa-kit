## v10.1.0-dev
- [Feature] Wire `quantityAvailableToReturn` from the Shopper Orders OMS expansion into the order-detail page. Adds a configurable `app.oms.returnEligibleStatuses` gate (default `['SHIPPED', 'DELIVERED']`, case-insensitive) and a new `getReturnableItems(order, returnEligibleStatuses)` helper at `app/utils/return-utils.js`. Renders a placeholder "Start return" CTA (disabled, "Returns coming soon" accessible label) on the order-detail page when the order's OMS status is eligible AND at least one item has `omsData.quantityAvailableToReturn > 0`. The full return flow lands in a follow-up story.

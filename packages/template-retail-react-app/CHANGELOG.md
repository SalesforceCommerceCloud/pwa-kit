## v10.1.0-dev
- [Feature] Add item-level order returns on the order detail page. Registered OMS shoppers can return eligible items via a modal with per-item quantity and reason selection, inline success/error feedback, and a status badge that reflects return progress (including partially-returned multi-unit lines). Gated behind `app.oms.enabled`. [#3904](https://github.com/SalesforceCommerceCloud/pwa-kit/pull/3904)

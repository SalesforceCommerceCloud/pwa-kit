...
- [Feature] Add `cc_conversationHistoryLimit` for the Commerce Client shopper-agent widget to control how many conversation entries are fetched when resuming a conversation. Defaults to `'200'` and is forwarded to the bundle as `conversationHistory.limit`. Storefronts with long conversations can raise this via `COMMERCE_AGENT_SETTINGS` to prevent messages from being dropped on refresh or navigation. The SCRT2 API caps the limit at 1000.

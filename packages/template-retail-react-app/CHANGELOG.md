...
- [Feature] Add guest order access Express endpoints (POST /api/order-lookup/verify, GET /api/order-lookup/order) and app.guestOrderLookup config block (disabled by default). Requires ECOM 26.8 and allowCookies/MRT_ALLOW_COOKIES.
- [Feature] Add flag-gated /order-lookup, /order-lookup/verify, /order-lookup/order routes and "Find Your Order" footer link (guestOrderLookup.enabled, off by default). Authenticated users redirected to /account/orders.

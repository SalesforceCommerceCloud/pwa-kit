...
- [Feature] Add `cc_pagePush` for the Commerce Client shopper-agent widget: when enabled, storefront content shifts aside (via layout padding on the shared `_app` container, desktop `lg`+ only) to make room for the open full-height side panel instead of being overlaid. Driven by the widget's `cimulate:ui-state-update` event so the layout stays in sync however the panel closes.

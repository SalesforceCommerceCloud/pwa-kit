...
- [Bugfix] Fix the Data Cloud recommender catalog event field name: `personalizationContextId` → `personalizationContentId`. The Data Cloud DLO schema, the official `@salesforce/cc-datacloud-typescript` SDK type, and Salesforce docs all define `personalizationContentId` (Content); the `Context` spelling was silently dropped on ingest, so the recommender UUID (`__recoUUID`) never landed.

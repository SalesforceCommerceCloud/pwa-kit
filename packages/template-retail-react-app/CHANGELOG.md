...
- [Enhancement] Support `INLINE_AGENT_WIDGET_SETTINGS` environment variable for inline agent widget configuration, matching the `COMMERCE_AGENT_SETTINGS` pattern used by the shopper agent. Credentials are read from the env var at runtime instead of being hardcoded in config. [#3998](https://github.com/SalesforceCommerceCloud/pwa-kit/pull/3998)

...
- [Feature] Add a `commerceClientLoadingMode` config for the Commerce Client shopper-agent widget (mirrors the SFCC cartridge's `cc_loadingMode`): `cdn` (default) loads the messaging UMD bundle from the external Cimulate CDN via `commerceClientScriptSourceUrl`; `static` loads it from the app's own bundled static assets via `commerceClientStaticAssetPath` (default `static/commerce-client/messaging.umd.js`), served same-origin. In `static` mode the cimulate.ai domain allowlist is skipped and the CDN URL is not required. Use the new `npm run copy:commerce-client` script to copy the widget bundle into `app/static/`.

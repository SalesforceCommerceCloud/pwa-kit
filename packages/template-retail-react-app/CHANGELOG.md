...
- [Bugfix] Send the shopper's active locale and currency to the shopper-agent chat session. The Commerce Client (Cimulate) provider previously sent no locale or currency at all; it now stamps the live selections onto `messagingConfig.routingAttributes` as `Locale` (from `useMultiSite().locale.id`) and `Currency` (from the active `useCurrency()` selection), so the SCRT2 session reflects what the shopper is actually browsing. These runtime values take precedence over any statically-configured `cc_routingAttributes`. The MIAW provider's hidden pre-chat `Currency` field now also reads the active `useCurrency()` selection instead of the static `locale.preferredCurrency` default, so a currency switch is reflected on the next agent conversation.

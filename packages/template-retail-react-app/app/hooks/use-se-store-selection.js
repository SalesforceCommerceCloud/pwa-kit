...
const useSeStoreSelection = () => {
    ...
    useEffect(() => {
        if (storeSearchData?.data && locationData && isProcessing) {
            ...
            if (selectedStore) {
                ...
                // Add SE flag to distinguish from manual selections and store search context
                window.localStorage.setItem(storeInfoKey, JSON.stringify({
                    ...
                    isSESelection: true, // Flag to indicate this was a SE selection
                    timestamp: Date.now(), // Add timestamp for potential future use
                    seSearchParams: seSearchParams // Store the search parameters that found this store
                }))
                ...
                window.dispatchEvent(new CustomEvent('seStoreSelected', {
                    detail: {
                        store: selectedStore,
    ...
    const processSeParameters = useCallback((urlParams) => {

...
async function importDwActiveData() {
    ...
    return await import('../assets/js/active-data').then(() => typeof globalThis.dw?.ac !== 'undefined', () => false)
}
...
function trackSearchHits(productSearchResult) {
    ...
    productSearchResult.hits.map(({id}) => ac?._capture?.({id, type: 'searchhit'}))
}
        ...
        async trackPage(siteId, localeId, currency) {
            try {
                ...
                if (canTrack) {
                    const ac = globalThis.dw?.ac
                    if (!ac?._analytics) {
                        ac?.setDWAnalytics?.(globalThis.dw?.__dwAnalytics?.getTracker?.(`${proxyBasePath}/ocapi/on/demandware.store/Sites-${siteId}-Site/${localeId}/__Analytics-Start`))
                    }

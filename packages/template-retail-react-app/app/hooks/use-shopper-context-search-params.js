 ...
 * to shopper context.
 */
export const useShopperContextSearchParams = (arraySearchParams = [SHOPPER_CONTEXT_SEARCH_PARAMS.CUSTOMER_GROUP_IDS]) => {
    const {search} = useLocation()
    const searchParamsObj = new URLSearchParams(search)
    
    const getSearchParam = (key, isList = false) =>
        searchParamsObj.has(key) ? { [key]: isList ? searchParamsObj.getAll(key) : searchParamsObj.get(key) } : {};
    const reduceParams = (keys) =>
        keys.reduce((acc, key) => ({ ...acc, ...getSearchParam(key, arraySearchParams.includes(key)) }), {});
    
    const customQualifiers = reduceParams(Object.values(SHOPPER_CONTEXT_CUSTOM_QUALIFIERS_SEARCH_PARAMS));
    const assignmentQualifiers = reduceParams(Object.values(SHOPPER_CONTEXT_ASSIGNMENT_QUALIFIERS_SEARCH_PARAMS));

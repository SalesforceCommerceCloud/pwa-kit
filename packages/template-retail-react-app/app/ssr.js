...
export async function getNotifyToken(apiParams) {
    ...
    // Private client: single round-trip via client_credentials grant.
    // Public client: PKCE guest flow — loginGuestUser handles authorize + code exchange
    // server-side; no browser needed, redirectURI just needs to be a registered value.
    const clientSecret = process.env.PWA_KIT_SLAS_CLIENT_SECRET
    const tokenResponse = clientSecret
        ...
        : await helpers.loginGuestUser({
              slasClient,
              parameters: {redirectURI: `${getAppOrigin()}/callback`}
          })
    _notifyToken = tokenResponse.access_token
    // Cache for 25 min — SLAS guest tokens last 30 min, 5 min buffer
...
export async function sendViaB2cCartridge(type, recipient, data, apiParams) {
    ...
    const url = `${proxy}/custom/pwakit-notify/v1/organizations/${encodeURIComponent(
        organizationId
    )}/sites/${encodeURIComponent(siteId)}/notify`

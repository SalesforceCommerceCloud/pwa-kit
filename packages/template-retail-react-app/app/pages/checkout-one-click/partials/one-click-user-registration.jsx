            ...
            <OtpAuth
                ...
                handleOtpVerification={async (otpCode) => {
                    try {
                        ...
                        let newBasketId = basket?.basketId
                        const hasBasketItem = basket?.productItems?.length > 0
                        if (hasBasketItem) {
                            await mergeBasket.mutateAsync({
                                parameters: {basketId: basket?.basketId, createDestinationBasket: true}
                            })
                            const refetched = await currentBasketQuery.refetch()
                            newBasketId =
                                refetched?.data?.basketId ||
                                refetched?.data?.basket_id ||
                                refetched?.data?.id ||
                                newBasketId
                        }
                        if (onRegistered) {

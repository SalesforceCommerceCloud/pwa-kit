...
describe('LoginState', () => {
  ...
  test('does not show passwordless login button if disabled', () => {
    const { queryByRole, queryByText } = renderWithProviders(<WrapperComponent isPasswordlessEnabled={false}/>)
  ...
  test('does not show social login buttons if disabled', () => {
    const { queryByRole, queryByText } = renderWithProviders(<WrapperComponent isSocialEnabled={false} idps={idps}/>)
    expect(queryByText('Or Login With')).not.toBeInTheDocument();

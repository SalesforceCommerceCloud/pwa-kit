...
describe('sf-payments-utils', () => {
    ...
    describe('createPaymentInstrumentBody', () => {
        ...
        test('does not include gatewayProperties when storePaymentMethod is false and futureUsageOffSession is false', () => {
            ...
            // gateway and gatewayProperties should not be included when not saving payment method
    ...
    describe('formatSavedPaymentMethods', () => {
        const mockStripeAccount = {
            accountId: 'stripe_account_123',
            vendor: 'Stripe'
        }

        test('returns empty array when paymentMethodReferences is undefined', () => {
            const result = formatSavedPaymentMethods(undefined, [mockStripeAccount])
            expect(result).toEqual([])
        })

        test('returns empty array when paymentMethodReferences is null', () => {
            const result = formatSavedPaymentMethods(null, [mockStripeAccount])
            expect(result).toEqual([])
        })

        test('returns empty array when paymentMethodReferences is empty', () => {
            const result = formatSavedPaymentMethods([], [mockStripeAccount])
            expect(result).toEqual([])
        })

        test('returns empty array when no matching account found', () => {
            const paymentMethodReferences = [
                {
                    paymentReference: {
                        paymentReferenceId: 'ref_123',
                        paymentMethodType: 'card',
                        gatewayProperties: {
                            stripe: {
                                last4: '4242',
            ...
            const result = formatSavedPaymentMethods(paymentMethodReferences, [])
            expect(result).toEqual([])
        })

        test('skips references without paymentReferenceId', () => {
            const paymentMethodReferences = [
                {
                    paymentReference: {
                        paymentMethodType: 'card',
            ...
            const result = formatSavedPaymentMethods(paymentMethodReferences, [mockStripeAccount])
            expect(result).toEqual([])
        })

        test('formats Stripe card payment method correctly', () => {
            const paymentMethodReferences = [
                {
                    paymentReference: {
                        paymentReferenceId: 'ref_123',
        ...
        test('formats Stripe payment method with missing optional fields', () => {
            const paymentMethodReferences = [
                {
                    paymentReference: {
                        paymentReferenceId: 'ref_456',
                        paymentMethodType: 'card',
                        gatewayProperties: {
                            stripe: {
                                last4: '1234',
            ...
            expect(result).toHaveLength(1)
            expect(result[0].id).toBe('ref_456')
            expect(result[0].gatewayTokenId).toBe('ref_456') // Falls back to paymentReferenceId
            expect(result[0].gatewayCustomerId).toBeNull()
            expect(result[0].expiryMonth).toBeNull()
            expect(result[0].expiryYear).toBeNull()
        })

        test('uses paymentReferenceId as gatewayTokenId when paymentMethodId is missing', () => {
            const paymentMethodReferences = [
                {
                    paymentReference: {
                        paymentReferenceId: 'ref_789',
                        paymentMethodType: 'card',
                        gatewayProperties: {
                            stripe: {
                                last4: '5678',
            ...
            expect(result[0].gatewayTokenId).toBe('ref_789')
        })

        test('handles different payment method types', () => {
            const paymentMethodReferences = [
                {
                    paymentReference: {
                        paymentReferenceId: 'ref_sepa',
            ...
            expect(result[0].type).toBe('sepa_debit')
            expect(result[0].name).toBe('Saved Payment Method') // Non-card types use fallback
        })

        test('defaults paymentMethodType to card when missing', () => {
            const paymentMethodReferences = [
                {
                    paymentReference: {
                        paymentReferenceId: 'ref_default',
                        gatewayProperties: {
                            stripe: {
                                last4: '9999',
                                brand: 'Visa'
        ...
        test('skips references without Stripe properties', () => {
            const paymentMethodReferences = [
                {
                    paymentReference: {
                        paymentReferenceId: 'ref_no_stripe',
                        paymentMethodType: 'card',
                        gatewayProperties: {}
                    }
                }
        ...
        test('handles multiple payment method references', () => {
            const paymentMethodReferences = [
                {
                    paymentReference: {
                        paymentReferenceId: 'ref_1',
                        paymentMethodType: 'card',
                        gatewayProperties: {
                            stripe: {
                                last4: '1111',
            ...
            expect(result[0].last4).toBe('1111')
            expect(result[1].id).toBe('ref_2')
        ...
        test('handles mixed valid and invalid references', () => {
            ...
            expect(result).toHaveLength(1)
            expect(result[0].id).toBe('ref_valid')
        })

        test('handles lowercase brand names correctly', () => {
            const paymentMethodReferences = [
                {
                    paymentReference: {
                        paymentReferenceId: 'ref_lowercase',
                        paymentMethodType: 'card',
                        gatewayProperties: {
                            stripe: {
                                last4: '4242',
                                brand: 'VISA' // Uppercase
                            }
                        }
                    }
                }
            ...
            expect(result[0].network).toBe('visa') // Should be lowercase
        })
    })
})

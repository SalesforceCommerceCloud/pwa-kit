...
const CommerceClientAgentWindow = ({commerceAgentConfiguration}) => {
    ...
    // Commerce Client JWT in the cim_af_ct_* storage key) to the SLAS SHOPPER
    ...
    /**
     * Extract the Commerce Client JWT from storage.
     *
     * The Commerce Client (Cimulate) widget persists its continuation token to
     * the `cim_af_ct_${orgId}_${esDeveloperName}` key (value shape:
     * {accessToken, lastEventId, storedAt}), and `accessToken` is the JWT we
     * present to the authlink endpoint. The widget writes this key before it
     * dispatches `onCimulateWidgetReady`, so it is available by the time the
     * auth-link flow runs.
     *
     * Checks localStorage first, then sessionStorage. Returns null if the token
     * is not present yet.
     */

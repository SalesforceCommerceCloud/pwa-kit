...
const AdyenCheckout = loadable(() => import('@adyen/adyen-web'), {
    ssr: false
})
...
export const ApplePayExpress = () => {
    ...
    const [adyenStylesLoaded, setAdyenStylesLoaded] = useState(false)
    ...
    useEffect(() => {
        // Dynamically load Adyen CSS only when component mounts
        import('@adyen/adyen-web/dist/adyen.css')
            .then(() => {
                setAdyenStylesLoaded(true)
            })
            .catch((error) => {
                console.warn('Failed to load Adyen styles:', error)
                setAdyenStylesLoaded(true) // Continue anyway
            })
    }, [])

    const {adyenEnvironment, adyenPaymentMethods, fetchShippingMethods} = useAdyenExpressCheckout()

    const createCheckout = async () => {
        if (!adyenStylesLoaded) {
            return
        }

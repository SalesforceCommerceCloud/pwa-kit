...
import {withCommerceSdkReactHookData} from '.'
...
describe('withCommerceSdkReactHookData enhanced component', function () {
    test('renders placeholder when hook is loading', async () => {
        const hookThatNeverLoads = createMockHook({data: undefined, isLoading: true})
        const EnhancedComponent = withCommerceSdkReactHookData(MockedComponent, {
    ...
    test('renders with data when hook is loaded', async () => {
        ...
        const EnhandedComponent = withCommerceSdkReactHookData(MockedComponent, {

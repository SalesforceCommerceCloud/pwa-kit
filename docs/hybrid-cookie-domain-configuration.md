...
- **Client-side cookies** set by `commerce-sdk-react` (`CookieStorage`) — including `usid`, `cc-nx-g` / `cc-nx` in non-HttpOnly mode, and the `dnt` cookie.
- **Server-side cookies** set by the SLAS proxy when `enableHttpOnlySessionCookies` is on — every cookie listed in the storage-mapping table (`cc-at`, `cc-nx-g`, `cc-nx`, `cc-at-expires`, `cc-nx-expires`, `customer_id`, `customer_type`, `enc_user_id`, `id_token`, `idp_access_token`, `idp_refresh_token`, `uido`, `usid`).
...
## 2. Configure the cookie domain on ECOM

The ECOM-side cookie domain for hybrid sessions is configured in Business Manager under:

**Merchant Tools → Site Preferences → Hybrid Auth Settings**

The cookie-domain setting accepts a numeric level that controls how broadly ECOM scopes the `dwsid` (and other hybrid session) cookies. Only two values are valid:
...
Run through this list once both PWA Kit and ECOM are configured. You can do most of this in Chrome DevTools → Application → Cookies.

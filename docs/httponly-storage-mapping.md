...
| `dwsid` (hybrid only) | cookie `dwsid` | cookie `dwsid` | Set by ECOM | ECOM owns this cookie; PWA Kit only reads it and forwards it as the `sfdc_dwsid` header to maintain server affinity. |
